...
export function saveProjectGithubCredentials(projectPath, { username, token, remote }) {
  ...
  ensureClaudeCredentialsIgnored(projectPath);
}

/**
 * Garantiza que .claude-credentials esté en el .gitignore del repo dado.
 * Si .gitignore no existe lo crea. Si ya tiene la entrada no hace nada.
 * Llamar siempre durante el setup de cada repo, con o sin token de proyecto.
 *
 * @param {string} repoPath
 */
export function ensureClaudeCredentialsIgnored(repoPath) {
  const gitignorePath = path.join(repoPath, '.gitignore');
  if (fs.existsSync(gitignorePath)) {
    const gi = fs.readFileSync(gitignorePath, 'utf8');
    if (!gi.includes('.claude-credentials')) {
      fs.appendFileSync(gitignorePath, '\n# Claude Code credentials (generado por workspace-template)\n.claude-credentials\n', 'utf8');
    }
  } else {
    fs.writeFileSync(gitignorePath, '# Claude Code credentials (generado por workspace-template)\n.claude-credentials\n', 'utf8');
  }
}
...
export async function resolveCredsFromRepo(repoPath) {
  ...
  // 4. ~/.git-credentials — siempre validar acceso al repo, sin importar cuántas cuentas haya.
  //    Una sola cuenta en el store no garantiza que sea la correcta para este repo.
  ...
  // 5. Sesión activa de gh CLI — solo si tiene acceso real al repo
  if (!token && remoteOwner && remoteRepo) {
    try {
      const { stdout: tokenOut } = await execa('gh', ['auth', 'token']);
      const ghToken = tokenOut.trim();
      if (ghToken) {
        const { stdout: accessOut } = await execa('gh', ['api', `repos/${remoteOwner}/${remoteRepo}`, '--jq', '.permissions.push'], {
          env: { ...process.env, GH_TOKEN: ghToken },
        });
        if (accessOut.trim() === 'true') {
    ...
    } catch { /* gh no instalado, sin sesión, o sin acceso al repo */ }
  }
