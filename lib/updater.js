 ...
 * Retorna un diff con entradas: new, updated, unchanged, customized.
 */
export async function computeDiff(workspacePath) {
  const diff = {
    skills: { new: [], updated: [], unchanged: [], customized: [] },
    rules:  { new: [], updated: [], unchanged: [], customized: [] },
  };

  const installed = readInstalledVersion(workspacePath) ?? { skills: {}, rules: {} };
  ...
  for (const kind of ['skills', 'rules']) {
    ...
    for (const file of srcFiles) {
      ...
      const originalHash = installed[kind]?.[file];

      if (srcHash === dstHash) {
        diff[kind].unchanged.push({ file, srcHash });
      } else if (originalHash && originalHash !== dstHash) {
        // El usuario modificó el archivo después de la instalación
  ...
  try {
    for (const kind of ['skills', 'rules']) {
      for (const file of selections[kind] ?? []) {
        ...
        const dstPath = path.join(workspacePath, '.claude', kind, file);

        fs.mkdirSync(path.dirname(dstPath), { recursive: true });
        fs.copyFileSync(srcPath, dstPath);

        versionData[kind] ??= {};
        versionData[kind][file] = await hashFile(srcPath);
        applied++;
      }
    }
    ...
    spinner.succeed(`${applied} archivo(s) actualizado(s)`);
    return applied;
  } catch (err) {
...
export async function runUpdate(workspacePath) {
  ...
  const totalPending = sk.new + sk.updated + sk.customized + ru.new + ru.updated + ru.customized;
  if (totalPending === 0) {
  ...
  const total = selections.skills.length + selections.rules.length;
  if (total === 0) {
