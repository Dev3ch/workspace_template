 ...
 *   El dev lo borró por algo. Si lo quiere de vuelta, tiene que pedirlo explícitamente.
 * - `new` solo aplica a archivos del template que nunca estuvieron instalados aquí
 *   (no figuran en el registry).
...
export async function computeDiff(workspacePath) {
  ...
  for (const kind of ['skills', 'rules', 'scripts']) {
    ...
    // Detectar archivos instalados que ya no existen en el template (upstream-removed)
    ...
    for (const file of srcFiles) {
      ...
      if (!fs.existsSync(dstPath)) {
        if (originalHash) {
          // Estuvo instalado, ya no está → el dev lo borró a propósito.
          // No reinstalar automáticamente.
          diff[kind].deletedByUser.push({ file, srcHash, originalHash });
        } else {
          // Nunca estuvo aquí → es un archivo nuevo del template upstream.
...
export async function runUpdate(workspacePath) {
  ...
  if (totalPending === 0) {
    if (sk.deletedByUser + ru.deletedByUser + sc.deletedByUser > 0) {
  ...
  for (const kind of ['skills', 'rules', 'scripts']) {
    const choices = [
      ...
      // Borrados localmente: ofrecer reinstalar, pero por defecto NO
