...
export async function computeDiff(workspacePath) {
  ...
  for (const kind of ['skills', 'rules']) {
    ...
    const srcNames = new Set(srcFiles.map((f) => f.replace(/\.md$/, '')));
    ...
    for (const file of srcFiles) {
      ...
      // Skills usan <name>/SKILL.md; rules usan <name>.md
      const dstPath = kind === 'skills'
        ? path.join(dstDir, skillName, 'SKILL.md')
      ...
      const originalHash = installed[kind]?.[skillName] ?? installed[kind]?.[file];
    ...
    for (const kind of ['skills', 'rules']) {
      for (const entry of selections[`${kind}Removed`] ?? []) {
        ...
        if (kind === 'skills') {
          ...
          delete versionData.rules?.[entry.file];
        }
        removed++;
      }
    }
...
export async function runUpdate(workspacePath) {
  ...
  const totalPending = sk.new + sk.updated + sk.customized + sk.removed + ru.new + ru.updated + ru.customized + ru.removed;
  if (totalPending === 0) {
  ...
  const selections = { skills: [], rules: [], skillsRemoved: [], rulesRemoved: [] };
  ...
  const total = selections.skills.length + selections.rules.length + selections.skillsRemoved.length + selections.rulesRemoved.length;
  if (total === 0) {
